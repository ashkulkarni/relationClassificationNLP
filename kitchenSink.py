...
def read_features(feature_filename,all_tokens,data_filename,output_filename,extraFeatures,more_feature_filename):
    count = 0
    with open(feature_filename) as featurefp, open(data_filename) as datafp,open(more_feature_filename) as morefeaturefp:
        for x, y,z in izip(featurefp, datafp,morefeaturefp):
...
if __name__ == "__main__":
    ...
    extraFeatures=[]
    fdep = open("dependency_tree/train_dependency_12000.txt","r")
    for line in fdep:
        extraFeature=[]
        fdepLine = line
        l=fdepLine.split('\t')
    ...
    writeTemplate("train_depUnig_morefeatures_12000.arff",len(all_tokens))
    read_features("train_unigram_tokens_12000.txt",all_tokens,"train_with_entities_12000.tsv","train_depUnig_morefeatures_12000.arff",extraFeatures,"dependency_tree/train_words_dependency_12000.txt")
